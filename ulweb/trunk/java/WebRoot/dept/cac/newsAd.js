...
function nextAd(toNum){
	...
	var theTimer=setTimeout("nextAd()", 6000);
}
...
function dakularButtons(){if(buttonShow==1){
	...
	document.getElementById('newsTable').style.top="-3px";
	switch(buttonPos){
		case 1:{buttonX=200-imgUrl.length*20;buttonY=17;break;}
		case 2:{buttonX=150-imgUrl.length*20;buttonY=160;break;}
		case 3:{buttonX=imgUrl.length*20-100;buttonY=17;break;}
		case 4:{buttonX=imgUrl.length*20-100;buttonY=160;break;}
		default:{buttonX=175-imgUrl.length*20;buttonY=17;break;}
	}

	for(i=1;i<=imgUrl.length-1;i++){
		dakularCode=dakularCode+"<div id='bt"+i+"' style='display:inline; height=12px; width:20px; border:1px solid #000000; background-color:#CCCCCC; color:#000000; font-size:12px; line-height:14px; filter: Alpha(Opacity=100);' align='center' onMouseOver=this.style.backgroundColor='#FFFFFF';this.style.color='#000000';this.style.cursor='hand' onMouseOut=checkButton(); onClick='clearTimeout(theTimer);nextAd("+i+");'>&nbsp;"+i+"&nbsp;</div>";
	}
	dakularCode="<div style='display:inline; position:absolute;right:50px; top:"+buttonY+"px; z-index:1;'>"+dakularCode+"</div>"
