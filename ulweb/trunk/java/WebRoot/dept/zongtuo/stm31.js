...
function stm_em()
{
	...
	for(var ppi=0;ppi<l;ppi++)
	{
		...
		if(p.isst&&m.maln!="left")
			ph="<TABLE width=100% STYLE='border:none;padding:0px;' CELLPADDING=0 CELLSPACING=0 ALIGN="+m.maln+"><TD >"+ph;
...
function stpbtx(p)
{
	...
	else
	{
		...
		s+=st_ttb ? "<TABLE  CELLPADDING=0 CELLSPACING=0" : "<DIV ";
		s+=stppev(p);
		s+=" ID="+p.ids;
		s+=" STYLE='";
		if(nIEM)
...
function stittx(i)
{
	...
	else
	{
		...
		else
		{
			if(p.pver&&p.plmw||!p.pver&&i.iicw)
			{
				s+="<TD  NOWRAP ALIGN=CENTER VALIGN=MIDDLE HEIGHT=100%";
			...
			s+=" ALIGN="+i.ihal;
			s+=" VALIGN="+i.ival+">";
			s+="<FONT  ID="+i.ids+"TX STYLE=\""+sttcss(i,0)+"\">";
			if(i.ityp==2)
