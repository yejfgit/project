...
<%
		...
		List<UlContent> newspaper = (List<UlContent>)request.getAttribute("newspaper");
		...
		String[] imgPath2 = {"npBgImg1.gif", "npBgImg2.gif"};
		String npBgImg = "";
		if(c.getColumnName().equals("合众保险报")){
			npBgImg = "images/pinxuan/" + imgPath2[0];
		}else if(c.getColumnName().equals("合众视点")){
			npBgImg = "images/pinxuan/" + imgPath2[1];
		}else{
			npBgImg = "images/pinxuan/" + imgPath2[(c.getColumnId() % 2)];
		}
  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="220" valign="top"><div style="height:300px;">
          <div style="margin:2px; padding:2px; border:solid #999999 1px;">
 
 				<div style=" height:14px; background:url(images/pinxuan/sp-colBg.gif) repeat-y; padding:3px 0px 3px 0px; text-align:center; color:#666666; font-size:15px; font-weight:bold;">合众内刊</div>
		 ...
		 <div style="padding:10px 10px 10px 25px;">
			...
			<tr>
              ...
              <td valign="middle" class="colNpText"><a href="pinxuan.do?method=subPage&cName=newspaper&columnId=<%=columnList.get(i).getColumnId()%>" class="colNpText"><%=columnList.get(i).getColumnName()%></a></td>
            </tr>
        ...
        <td valign="top" background="<%=npBgImg%>"><div style="border-left:#CCCCCC solid 1px;">
		<div style="height:23px; width:540px; margin-left:10px; border-bottom:#CCCCCC solid 1px; padding:1px 20px 1px 0px; text-align:right"><div style="float:right; height:20px; padding-top:5px; font:宋体 12px; color:#666666">&nbsp;&gt;&gt;&nbsp;合众内刊&nbsp;&gt;&gt;&nbsp;<%=c.getColumnName()%></div><div style=" float:right; width:30px; height:20px; text-align:center; "><a href="pinxuan.do?method=index"><img src="images/pinxuan/index.gif" width="30" height="20" border="0"></a></div>
		  </div>
		<div style="width:540px; height:200px; margin-left:5px; padding:10px 10px 10px 20px; line-height:15px; color:#666666;">
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			//-----------------------------------			
		  	...
		  	for(int i = 0; i < (newspaper == null ? 0 : newspaper.size()); i++) {
			
				//-----------------------------------
				List<UlContent> list = newspaper;
				att = "";
				attStart = 0;
		  ...
		  %>
            <tr>
              ...
              <td valign="top"><div style=" padding:3px; border-bottom:dotted #CCCCCC 1px;" class="listText"><a href="<%=path%>" target="_blank" class="listText"><%=(newspaper.get(i).getContentName().length() < 30 ? newspaper.get(i).getContentName() : newspaper.get(i).getContentName().substring(0, 30) + "...")%></a><%=att%></div></td>
              <td width="100" valign="bottom"><div style="padding:3px;border-bottom:dotted #CCCCCC 1px;"><%=newspaper.get(i).getDay()%></div></td>
            </tr>
            <%  } %>
          </table>
		  </div>
