...
<%
		...
		List<UlContent> system = (List<UlContent>)request.getAttribute("system");
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
		...
		<div style="height:23px; width:540px; margin-left:10px; border-bottom:#CCCCCC solid 1px; padding:1px 20px 1px 0px; text-align:right">
		  <div style="float:right; height:20px; padding-top:5px; font:宋体 12px; color:#666666">&nbsp;&gt;&gt;&nbsp;制度建设&nbsp;</div>
		...
		<div style="width:540px; height:200px; margin-left:5px; padding:10px 10px 10px 20px; line-height:15px; color:#666666;">
		  ...
		  <%
			...
			//-----------------------------------		  
		  	...
		  	for(int i = 0; i < (system == null ? 0 : system.size()); i++) {
			
				//-----------------------------------
				List<UlContent> list = system;
				att = "";
				attStart = 0;
		  ...
		  %>
            ...
            <tr>
              ...
              <td valign="top"><div style=" padding:3px; border-bottom:dotted #CCCCCC 1px;" class="listText"><a href="<%=path%>" target="_blank" class="listText"><%=(system.get(i).getContentName().length() < 30 ? system.get(i).getContentName() : system.get(i).getContentName().substring(0, 30) + "...")%></a><%=att%></div></td>
              <td width="100" valign="bottom"><div style="padding:3px;border-bottom:dotted #CCCCCC 1px;"><%=system.get(i).getDay()%></div></td>
            </tr>
