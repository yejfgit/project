...
public class GenSql {
	//表名
	private static TableName tabname =  TableName.SS_YW0000;
	//是否需要完整的SQL查询语句
	private static final boolean isNeedAllSql = false;
	//是否需要显示注释
	private static final boolean isNeedMark = true;
	/**
	 * @param args
	...
	public static void main(String[] args) {
		...
		try {
			...
			if(isNeedMark)  map = GenSql.getComMap();
			...
			for (int i = 1; i <= rsmd.getColumnCount(); i++) {
				...
				System.out.println(tabname+"."+rsmd.getColumnName(i)  
						+"  " + tabname + "_"+rsmd.getColumnName(i)+","
						+ mark);
			}
