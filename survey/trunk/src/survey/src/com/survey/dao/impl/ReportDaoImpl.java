	...
	public List getTimesList(String startDate, String endDate, int deptId,
		...
		sb
				.append("select d.full_name deptFullName,u.real_name userRealName,"
						+ "m.total_score xzScore,s.name surveyName," 
						+ "wd.wdScore wdScore , m.total_score+wd.wdScore  totalScore "
						+ "from t_sv_survey s,t_um_user u,t_sv_mission m,t_um_organ d," 
						+ "(select SUM(t.score) wdScore,mm.id missionId "
								+"from t_sv_text_answer t,t_sv_answer a,t_sv_mission mm "
								+"where t.answer_id = a.id "
								+"and a.mission_id = mm.id "
								+"group by mm.id) wd "
						+ "where u.um_user_id = m.user_id "
						+ "and u.um_organ = d.um_organ_id "
						+ "and s.id = m.survey_id "
						+ "and wd.missionId(+) = m.id "
