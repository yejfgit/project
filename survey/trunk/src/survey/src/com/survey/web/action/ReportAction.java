...
//		 查询条件参数
		...
		if (request.getParameter("export") != null && request.getParameter("export").equals("xls")) {
			...
			sheet.addCell(new Label(2, 0, "选择题分数"));
			sheet.addCell(new Label(3, 0, "问答题分数"));
			sheet.addCell(new Label(4, 0, "分数"));
			
			for (int i = 0; i < reportList.size(); i++) {
				ReportTimesVO rt = (ReportTimesVO) reportList.get(i);
				BigDecimal rtWdScore = rt.getWdScore();
				double WdScore;
				if(rtWdScore==null){
					WdScore = 0;
				}else{
					WdScore = rtWdScore.doubleValue();
				}
				
				BigDecimal rtTtScore = rt.getTotalScore();
				double totalScore;
				if(rtTtScore==null){
					totalScore = 0;
				}else{
					totalScore = rtTtScore.doubleValue();
				}
				
				sheet.addCell(new Label(0, (i + 1), rt.getDeptFullName()));
				sheet.addCell(new Label(1, (i + 1), rt.getUserRealName()));
				sheet.addCell(new Number(2, (i + 1), rt.getXzScore().doubleValue()));
				sheet.addCell(new Number(3, (i + 1), WdScore));
				sheet.addCell(new Number(4, (i + 1), totalScore));
			}
...
//				sheet.addCell(new Label(1, (i + 1), rt.getUserRealName()));
//				sheet.addCell(new Number(2, (i + 1), rt.getXzScore().doubleValue()));
//				sheet.addCell(new Number(3, (i + 1), rt.getWdScore().doubleValue()));
//				sheet.addCell(new Number(4, (i + 1), rt.getTotalScore().doubleValue()));
//			}
//			
//			book.write();
//			book.close();
